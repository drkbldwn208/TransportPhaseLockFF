...
#include "dwell_fcw_streamer_64.h"

static axis_cmd64_t make_command(ap_uint<32> fcw, ap_uint<32> dwell_cycles, bool last) {
    axis_cmd64_t word;
...
static void tick(
    hls::stream<axis_cmd64_t> &commands,
    bool start,
    ap_uint<32> &fcw_out,
    ap_uint<1> &active,
    ap_uint<1> &dwell_wait,
    int cycle) {
    dwell_fcw_streamer_64(commands, start, &fcw_out, &active, &dwell_wait);
...
int main() {
    hls::stream<axis_cmd64_t> commands("commands");

    const ap_uint<32> idle_fcw = 0x0;
