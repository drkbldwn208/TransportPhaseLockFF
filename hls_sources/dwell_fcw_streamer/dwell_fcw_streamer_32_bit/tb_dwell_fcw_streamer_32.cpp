...
#include "dwell_fcw_streamer_32.h"

static axis_cmd32_t make_command(ap_uint<16> fcw, ap_uint<16> dwell_cycles, bool last) {
    axis_cmd32_t word;
    word.data.range(31, 16) = fcw;
    word.data.range(15, 0) = dwell_cycles;
    word.keep = 0xF;
    word.strb = 0xF;
    word.last = last ? 1 : 0;
    return word;
...
static int check_equal_u16(const char *name, ap_uint<16> got, ap_uint<16> expected) {
    if (got != expected) {
...
static void tick(
    hls::stream<axis_cmd32_t> &commands,
    bool start,
    ap_uint<16> &fcw_out,
    ap_uint<1> &active,
    ap_uint<1> &dwell_wait,
    int cycle) {
    dwell_fcw_streamer_32(commands, start, &fcw_out, &active, &dwell_wait);
...
int main() {
    ...
    */
    ...
    errors += check_equal_u16("idle FCW", fcw_out, idle_fcw);
    if (active != 0) {
    ...
    */
    ...
    tick(commands, false, fcw_out, active, dwell_wait, cycle++);
    tick(commands, false, fcw_out, active, dwell_wait, cycle++);
    errors += check_equal_u16("first command FCW", fcw_out, fcw_a);
    ...
    */
    tick(commands, false, fcw_out, active, dwell_wait, cycle++);
    tick(commands, false, fcw_out, active, dwell_wait, cycle++);
    errors += check_equal_u16("second command FCW", fcw_out, fcw_b);

    tick(commands, false, fcw_out, active, dwell_wait, cycle++);
    tick(commands, false, fcw_out, active, dwell_wait, cycle++);
    errors += check_equal_u16("return-to-idle FCW", fcw_out, idle_fcw);
