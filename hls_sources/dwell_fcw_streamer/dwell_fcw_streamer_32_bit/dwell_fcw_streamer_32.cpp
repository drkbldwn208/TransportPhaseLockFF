...
#pragma HLS PIPELINE II=1
    ...
    static ap_uint<16> current_fcw = DWELL_FCW_STREAMER_IDLE_FCW;
    static ap_uint<16> dwell_remaining = 0;
    ...
    *dwell_wait = ((state == STATE_RUN) && (dwell_remaining != 0)) ? 1 : 0;
    ...
    if (current_word_was_last && dwell_remaining == 0) {
        ...
        axis_cmd32_t command_word = s_axis_cmd.read();
        ap_uint<32> payload = command_word.data;

        current_fcw = payload.range(31, 16);
        dwell_remaining = payload.range(15, 0);
